...
load('Floresphyfilt.RData')

#load('FloresNetFits.RData')
...
se.is <- spiec.easi(phy_taxfilt, method='ising', nlambda=50,
            lambda.min.ratio=1e-1, lambda.log=FALSE,
            pulsar.params=list(ncores=38, rep.num=35))
...
ranks <- round(exp(seq(log(2), log(64), len=6)))
...
save(se.gl, se.mb, se.is, se.poi, se.slr, file='FloresNetFits.RData')
