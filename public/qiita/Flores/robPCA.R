...
tmp <- build_pca('FloresNetFits.RData')
scores  <- tmp$scores
Rscores <- tmp$Rscores
dims    <- tmp$dims
...
rm_fact <- 'ambiguoustimepoint'

map <- process_map(col_fact, col_num, rm_fact=rm_fact)
## attach depth data
...
pvals$n_features <- length(col_num) + length(col_fact) + 2
