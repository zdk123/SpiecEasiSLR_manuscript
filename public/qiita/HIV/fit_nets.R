...
se.poi <- spiec.easi(phy_taxfilt, method='poisson', nlambda=100,
            lambda.min.ratio=5e-3, lambda.log=FALSE,
            pulsar.params=list(ncores=38, rep.num=35))

save(se.gl, se.mb, se.is, se.poi, file='NetFits.RData')

#ranks <- round(exp(seq(log(2), log(32), len=6)))
ranks <- round(exp(seq(log(2), log(64), len=8)))
...
save(se.gl, se.mb, se.is, se.poi, se.slr, file='HIVNetFits.RData')
